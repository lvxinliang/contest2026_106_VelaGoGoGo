...
#define CIR_TX_DBG(fmt, args...) rcinfo(fmt, ##args)
#define CIR_TX_ERR(fmt, arg...) rcerr(fmt, ##arg)
...
cir_tx_fifo_full(struct sunxi_cir_tx_t *cir_tx) {
  unsigned int reg_val;
  unsigned long reg_addr = SUNXI_IRTX_PBASE;
...
// fix unused-function
__attribute__((__unused__)) static int hal_cir_reg_clear(void) {
  unsigned long reg_addr = SUNXI_IRTX_PBASE;
...
hal_irqreturn_t hal_cir_tx_handler(void *dev_id) {
  ...
  // syslog(LOG_INFO, "hal_cir_tx_handler: dev_id=%p\n", dev_id);
...
static int run_length_encode(unsigned int *raw_data, unsigned char *txbuf,
                             unsigned int count) {
  unsigned int is_high_level = 0;
  unsigned int num = 0;

  if (count >= IR_TX_RAW_BUF_SIZE - 5) {
...
static int send_ir_code(void) {
  unsigned int reg_val, idle_threshold;
  unsigned int sent, first, avail, guard;
  unsigned long reg_addr = SUNXI_IRTX_PBASE;
  ...
  /* get idle threshold */
  idle_threshold =
      (readl(reg_addr + IR_TX_IDC_H) << 8) | readl(reg_addr + IR_TX_IDC_L);

  /* IR_TX_TR 写入的是本次要发送的"总字节数", 硬件据此从 FIFO 连续取数发送。
   * FIFO 深度仅 128, 但整机空调帧(如美的)编码后常达 ~130 字节, 原代码只填一次、
   * 超 128 就 "fail to packet" 直接放弃(表现: ir_tx_test 短帧能发, "打开空调"
   * 长帧不发)。这里改为: 先填满 FIFO, 触发发送, 再随 FIFO 排空补写剩余字节,
   * 从而支持任意长度(≤ IR_TX_RAW_BUF_SIZE)的帧。≤128 的短帧行为与原来一致。 */
  writel((cir_raw_buffer.tx_dcnt - 1), reg_addr + IR_TX_TR);

  first = (cir_raw_buffer.tx_dcnt < IR_TX_FIFO_SIZE)
          ? cir_raw_buffer.tx_dcnt : IR_TX_FIFO_SIZE;
  for (sent = 0; sent < first; sent++)
    writel(cir_raw_buffer.tx_buf[sent], reg_addr + IR_TX_FIFO_DR);
  ...
  if (IR_TX_CYCLE_TYPE) {
    ...
    /* 长帧: FIFO 有空位就补写剩余字节。guard 防止异常时死循环。 */
    guard = 0;
    while (sent < cir_raw_buffer.tx_dcnt && guard++ < 2000000) {
      avail = readl(reg_addr + IR_TX_TACR);   /* 空闲槽位数 */
      while (avail > 0 && sent < cir_raw_buffer.tx_dcnt) {
        writel(cir_raw_buffer.tx_buf[sent++], reg_addr + IR_TX_FIFO_DR);
        avail--;
      }
    }
  ...
  /* wait idle finish */
  while ((readl(reg_addr + IR_TX_ICR_H) << 8 | readl(reg_addr + IR_TX_ICR_L)) <
         idle_threshold) {
    // CIR_TX_DBG("wait idle\n");
  }

  /* Clear TCS (Force Trigger) to prevent re-triggering or stuck state */
  reg_val = readl(reg_addr + IR_TX_CR);
  reg_val &= ~(0x01 << 7);
  writel(reg_val, reg_addr + IR_TX_CR);

  /* Clear Transmit Enable bit to ensure pin returns to idle/low */
  reg_val = readl(reg_addr + IR_TX_GLR);
  reg_val &= ~0x02;
  writel(reg_val, reg_addr + IR_TX_GLR);
...
void hal_cir_tx_xmit(unsigned int *txbuf, unsigned int count) {
  ...
  /* Ensure we end with a Space (Low/Off) to turn off the LED */
  /* If the last item was a Pulse (i.e. count is Odd, so valid indices were 0..even), 
     we need to append a dummy Space. */
  if ((count % 2) != 0) {
      if (index < IR_TX_RAW_BUF_SIZE) {
          /* Append a Space (Bit 7 = 0). 
             Duration = 1 (minimal unit) * 32 (default three_pulse_cycle) / 3 / clk...
             Actually duration just needs to be non-zero for the HW to process it and switch level.
             Let's use a small valid value. 
             If run_length_encode logic: val = (duration * 3) / 32. 
             We bypass calculation and write directly. 
             Let's write a small duration space. 
             0x00 | 0x01 means Space with duration count 1.
          */
          cir_raw_buffer.tx_buf[index] = 0x01; 
          index++;
      }
  }
...
void hal_cir_tx_set_duty_cycle(int duty_cycle) {
  ...
  if (duty_cycle < 30) {
    reg_val |= 0x40;
    CIR_TX_DBG("set duty cycle to 25%%\n");
  } else if (duty_cycle < 40) {
    reg_val |= 0x20;
    CIR_TX_DBG("set duty cycle to 33%%\n");
  } else {
    CIR_TX_DBG("set duty cycle to 50%%\n");
  }
...
static int hal_cir_clk_init(struct sunxi_cir_tx_t *cir_tx) {
  ...
  if (cir_tx->g_clk_id) {
    ...
    /* 功能修复: 载波分频公式(hal_cir_tx_set_carrier)硬编码假设模块时钟 =
     * IR_TX_CLK(12MHz), 但原代码只 enable 未 set_rate, 实际默认时钟(R528 常见
     * 24MHz)会使载波成倍偏离 38kHz, 令 38kHz 解调器(空调/学习工具)收不到。
     * 显式设定为 IR_TX_CLK 使载波回到 38kHz。 */
    hal_clk_set_rate(cir_tx->gclk, IR_TX_CLK);
  }
...
cir_tx_status_t hal_cir_hw_init(struct sunxi_cir_tx_t *cir_tx) {
  ...
  if (hal_cir_clk_init(cir_tx)) {
    CIR_TX_DBG("cir_tx_clk_init err!\n");
    return CIR_TX_CLK_ERR;
  } else {
    CIR_TX_DBG("cir_tx_clk_init success\n");
  }

  if (hal_cir_gpio_init(cir_tx)) {
    CIR_TX_DBG("cir_tx_gpio_init err!\n");
    err = CIR_TX_PIN_ERR;
    goto err0;
  } else {
    CIR_TX_DBG("cir_tx_gpio_init success\n");
  }

  hal_cir_reg_cfg();

  if (hal_request_irq(cir_tx->irq, hal_cir_tx_handler, "cir_tx", cir_tx) < 0) {
    CIR_TX_DBG("cir_tx request irq err\n");
    err = CIR_TX_IRQ_ERR;
    goto err1;
  } else {
    CIR_TX_DBG("cir_tx request irq success\n");
  }
...
err1:
  ...
  CIR_TX_DBG("cir_tx_gpio_exit err!\n");
err0:
  hal_cir_clk_exit(cir_tx);
  CIR_TX_DBG("cir_tx_clk_exit err!\n");
  return err;
}
...
static inline void sunxi_irtx_save_regs(struct sunxi_cir_tx_t *chip) {
  ...
  for (i = 0; i < ARRAY_SIZE(sunxi_irtx_regs_offset); i++)
    chip->regs_backup[i] =
        readl((unsigned long)(chip->reg_base) + sunxi_irtx_regs_offset[i]);
}
...
static inline void sunxi_irtx_restore_regs(struct sunxi_cir_tx_t *chip) {
  ...
  for (i = 0; i < ARRAY_SIZE(sunxi_irtx_regs_offset); i++)
    writel(chip->regs_backup[i],
           (unsigned long)(chip->reg_base) + sunxi_irtx_regs_offset[i]);
}
...
cir_tx_status_t hal_cir_tx_init(struct sunxi_cir_tx_t **cir_tx_ptr) {
  int ret;
  struct sunxi_cir_tx_t *cir_tx = 0;

  cir_tx = hal_malloc(128);
  if (cir_tx == NULL) {
    CIR_TX_ERR("cir_tx malloc error\n");
  }
  ...
  if (ret) {
    ...
    CIR_TX_DBG("cir_tx hardware init success\n");
  }
